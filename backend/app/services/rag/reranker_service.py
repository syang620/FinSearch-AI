"""
LLM-based Re-ranking Service for Financial Document Retrieval
...
class LLMReranker:
    """LLM-based document re-ranking service"""
    ...
    def rerank(
        ...
        """
        Re-rank all candidates and return top K
        ...
        for candidate in candidates:
            score, reasoning = self.score_candidate(query, candidate, query_intent)
...
# Import hybrid reranker for best performance
try:
    from app.services.rag.reranker_service_hybrid import HybridLLMReranker
    # Use hybrid reranker for optimal balance of efficiency and parallelism
    reranker = HybridLLMReranker(model="qwen2.5:0.5b", max_workers=4)
    logger.info("Using hybrid reranker (4 workers, batch processing)")
except ImportError:
    try:
        from app.services.rag.reranker_service_batch import BatchLLMReranker
        # Fallback to batch reranker for most efficient scoring (single LLM call)
        reranker = BatchLLMReranker(model="qwen2.5:0.5b")
        logger.info("Using batch reranker for maximum efficiency")
    except ImportError:
        try:
            from app.services.rag.reranker_service_parallel import ParallelLLMReranker
            # Fallback to parallel reranker with 4 workers
            reranker = ParallelLLMReranker(model="qwen2.5:0.5b", max_workers=4)
            logger.info("Using parallel reranker with 4 workers")
        except ImportError:
            # Final fallback to sequential reranker
            reranker = LLMReranker()
            logger.info("Using sequential reranker")